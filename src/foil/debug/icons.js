...
export function icon(name) {
  ...
  const element = declaredTemplate('icon') || legacyElement('ui-icon');
...
export function button(label, glyph, action, text = '') {
  ...
  const button = authored || legacyElement('ui-button');
  ...
  const prefix = authored ? button.querySelector('ui-icon') : icon(glyph);
  if (authored) prefix.setAttribute('name', iconNames[glyph]);
  if (text) prefix.slot = 'prefix';
  if (!authored) button.append(prefix);
  if (text) { const span = authored ? button.querySelector('span') : legacyElement('span'); span.hidden = false; span.textContent = text; if (!authored) button.append(span); }
