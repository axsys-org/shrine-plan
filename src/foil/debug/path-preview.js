...
const element = (tag, name = '', text = '') => {
  const node = legacyElement(tag); node.className = name; node.textContent = text; return node;
};

export function createMythPreview(view, href) {
  const authored = declaredTemplate('hover-myth');
  const myth = authored || element('sh-myth', 'wb-hover-myth');
  myth.setAttribute('variant', 'preview'); myth.setAttribute('data-mash-size', 'compact');
  myth.setAttribute('title', view.label || view.path);
  const path = authored ? myth.querySelector('.wb-hover-path') : element('a', 'wb-hover-path');
  path.textContent = view.path; path.slot = 'path'; path.href = href;
  if (!authored) myth.append(path);
  if (view.description && view.description.trim() !== view.label?.trim()) {
    const description = authored ? myth.querySelector('.wb-hover-description') : element('p', 'wb-hover-description');
    description.hidden = false; description.textContent = view.description; description.slot = 'description'; if (!authored) myth.append(description);
  }
  ...
  for (const slot of shown) {
    const limb = declaredTemplate('hover-slot') || element('sh-limb');
    const key = authored ? limb.querySelector('sh-slot') : element('sh-slot'); key.slot = 'label'; key.setAttribute('title', slot.key);
    const definition = authored ? key.querySelector('ui-button') : element('ui-button'); definition.textContent = slot.key;
    ...
    if (!authored) key.append(definition);
    const value = authored ? limb.querySelector('sh-pail') : element('sh-pail'); value.slot = 'value';
    ...
    const text = authored ? value.querySelector(reference ? 'a' : 'span') : element(reference ? 'a' : 'span', 'wb-hover-value');
    ...
    if (!authored) { value.append(text); limb.append(key, value); } myth.append(limb);
  }
  const metadata = authored ? myth.querySelector('.wb-hover-meta') : element('span', 'wb-hover-meta'); metadata.slot = 'meta';
  ...
  if (!authored) myth.append(metadata);
  if (shown.length) {
    const note = authored ? myth.querySelector('.wb-hover-note') : element('span', 'wb-hover-note');
    note.hidden = false; note.textContent = 'Preview · ' + shown.length + ' of ' + slotCount + ' slots';
    note.slot = 'annotation'; if (!authored) myth.append(note);
  }
  const open = authored ? myth.querySelector('.wb-hover-open') : element('a', 'wb-hover-open', 'Open record'); open.slot = 'actions'; open.href = href;
  open.setAttribute('aria-label', 'Open ' + view.path); if (!authored) myth.append(open);
  return myth;
}
...
export function createPathPreview({resolvePath, readPreview, initialView, renderRecord = createMythPreview}) {
  ...
  const card = declared ? document.querySelector('#wb-path-preview') : element('ui-preview-card'); card.id = 'wb-path-preview';
  ...
  if (!declared) document.querySelector('.debug-shell').append(card);
  ...
  function setContent(state, content) {
    ...
    const scroll = element('ui-scroll-area', 'wb-hover-scroll mash-scroll-quiet');
    scroll.setAttribute('label', 'Record preview'); scroll.setAttribute('mode', 'scrolling');
    scroll.setAttribute('orientation', 'vertical'); scroll.setAttribute('size', 'medium');
    scroll.append(content); card.replaceChildren(scroll);
    card.setAttribute('aria-busy', String(state === 'loading'));
    // A synchronous content change must not leave geometry sized for Loading.
    card.requestUpdate();
  }
  async function load(chosen) {
    ...
    if (chosen.path === '/log') {
      const note = declaredTemplate('context-note') || element('p', 'wb-context-note'); note.textContent = 'Open activity to inspect its entries.';
      setContent('unavailable', note); return;
    }
    const loading = declaredTemplate('context-note') || element('p', 'wb-context-note'); loading.textContent = 'Loading record…'; loading.setAttribute('role', 'status');
    ...
    try {
      ...
      const content = declaredTemplate('hover-error') || element('div', 'wb-hover-error');
      if (declared) {
      ...
      } else content.append(element('p', 'wb-context-note', 'Could not preview this path. ' + error.message),
        button('Retry preview', 'live', () => void load(chosen), 'Retry'));
