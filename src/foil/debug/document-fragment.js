...
export function bindDocumentFragment(main, view, {readPreview, attachValueInspection, requests}) {
  ...
  main.addEventListener('sh-path-row-toggle', async event => {
    ...
    try {
      ...
      if (!__DEBUG_LEGACY__ && !authored && !declared) throw new Error('Missing Grove path-preview declaration');
      ...
      // Grove-authored and compatibility fragments cross the same inert-read
      // boundary. Neither may import scripts, forms, handlers or external URLs.
