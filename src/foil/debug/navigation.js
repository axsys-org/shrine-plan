...
(() => {
  ...
  function report(message, failed = false, {
    ...
    const item = declaredTemplate('activity-entry') || legacyElement('li');
  ...
  // Move the controls, retaining their state and handlers. Mash owns the
  // composite keyboard contract; the server nav remains the no-JS fallback.
  const declared = !__DEBUG_LEGACY__ || Boolean(get('[data-grove-contract="debugger/v1"]'));
  if (!declared) {
  const previousHistoryNav = get('#debug-history-nav');
  const historyNav = legacyElement('ui-toolbar');
  historyNav.id = 'debug-history-nav';
  historyNav.setAttribute('label', 'Navigation history');
  historyNav.setAttribute('data-mash-size', 'small');
  for (const [id, label, glyph] of [
    ['debug-back', 'Back', 'back'],
    ['debug-forward', 'Forward', 'forward'],
  ]) {
    const button = get('#' + id);
    ...
    historyNav.append(button);
  }
  previousHistoryNav?.remove();
  get('.debug-header').prepend(historyNav);
  }
  ...
  prepare({ freshMain: true });
  // Compose the persistent session panel through the public workspace slot.
  // Keep the server-rendered footer as the no-JavaScript fallback.
  const sessionPanel = get('.debug-footer');
  if (!declared) {
    sessionPanel.slot = 'main-footer';
    get('#debug-workspace').append(sessionPanel);
  }
  sessionPanel.querySelector('details').open = true;
  const clearActivity = declared ? get('#debug-clear-activity') : legacyElement('ui-button');
  ...
  if (!declared) sessionPanel.append(clearActivity);
