...
export function attachValueInspection(pail, slot, {unknownContext = null} = {}) {
  ...
  const status = declaredTemplate('value-status') || legacyElement('span');
  ...
  const area = declaredArea || legacyElement('ui-scroll-area');
  ...
  const pre = declaredArea ? area.querySelector('pre') : legacyElement('pre'); if (!declaredArea) area.append(pre);
