...
const host = await readFile(resolve(root, 'debugger_view.foil'), 'utf8');
const debuggerHost = host;
...
const app = await readFile(resolve(root, 'application.grove'), 'utf8');
assert.match(app, /inspection =\s+@role/);
const instance = await readFile(resolve(root, 'instance.grove'), 'utf8');
...
const icons = await readFile(resolve(root,'icons.js'),'utf8');
...
for (const file of ['debugger.grove', 'chrome.grove', 'model.grove', 'document.grove']) {
