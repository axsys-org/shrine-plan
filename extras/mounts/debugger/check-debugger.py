...
Use nix develop path:./extras#debugger, or supply WISP, Node, pnpm, and an explicit
...
def check(args):
    ...
    if not args.backend_only and not (os.environ.get('MASH_ROOT') or os.environ.get('MASH_NIX_ROOT')):
        raise RuntimeError('Enter nix develop path:./extras#debugger or set MASH_ROOT to the paired Mash checkout.')
