...
from eden_mounts import select, prepare, runtime_specs, ScanBridge, runtime_literal
import hashlib
import json
import os
...
PROTECTED_PORT = 8138
ASSETS = {
    '/style.css': 'mounts/web/style.css',
    '/debug.js': 'mounts/debugger/debug.js',
    '/debug.css': 'mounts/debugger/debug.css',
    '/debug-mash.js': 'mounts/debugger/mash.js',
    '/debug-components.css': 'mounts/debugger/components.css',
}
...
def startup_source(entry, port, node_number, specs=(), scan_port=0, scan_token=""):
...
(define (launch ignored)
  ...
  (eden:start ("x" {node_number}) system resolve slot {port} {literal(specs)} {scan_port} {literal(scan_token)}))
...
def launch(args):
    with ExitStack() as cleanup:
        return launch_with(args, cleanup)

def launch_with(args, cleanup):
    ...
    names = {'start-srs': ['srs'], 'start-debug': ['debugger'],
             'start-debug-srs': ['debugger', 'srs']}[args.entry]
    selected = select(EXTRAS, names)
    prepared = prepare(selected, work, False, cleanup,
                       {'node': node, 'timeout': args.startup_timeout})
    specs = runtime_specs(prepared, source)
    scanner = cleanup.enter_context(ScanBridge(prepared, source))
    snapshot = work / 'snap'
    snapshot.mkdir()
    for name in ('data.mdb', 'pins.pack'):
    ...
    try:
        with (work / 'out.log').open('w') as output:
            ...
            process.stdin.write(startup_source(args.entry, port, node_number, specs, scanner.port, scanner.token))
            process.stdin.flush()
...
def seal_assets(path, asset_source):
    ...
    build = verify_asset_build(source / 'mounts/debugger') if (source / 'mounts/debugger').exists() else None
    copied = {}
...
def main():
    ...
    start.add_argument('--startup-timeout', type=int, default=2400)
    ...
    assets.add_argument('--source', type=Path, default=ROOT / 'src',
                        help='Reserved for compatibility; the seal records the startup asset snapshot')
